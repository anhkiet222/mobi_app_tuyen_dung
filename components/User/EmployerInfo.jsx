...
import { fetchDataCodeGender, updateUserInfo } from "../../api/userApi";
    ...
    const fetchGenderData = async () => {
      try {
        ...
        if (genderData) {
          ...
          const matchedItem = formattedData.find(
            (item) => item.label === userData.genderCodeValue
          );
          if (matchedItem) {
      ...
      try {
        ...
        if (data.code === 200) {
          setUserData({
            ...
            genderCodeValue:
              genderItems.find((item) => item.value === formData.genderCode)
                ?.label || "",
            email: formData.email,
          });
          Alert.alert(
        ...
        <View style={styles.infoContainer}>
          ...
          {isEditing ? (
            ...
            <Text style={styles.input}>
              {userData.genderCodeValue || "Chưa xác định"}
            </Text>
          )}
