  ...
  const fetchData = async (userId, page, isInitialLoad = false) => {
    ...
    try {
      ...
      if (data && data.code === 200) {
        ...
        setDataCv((prevData) =>
          isInitialLoad ? newData : [...prevData, ...newData]
        );
    ...
    return (
      <View style={styles.itemContainer}>
        ...
        <Text style={styles.itemText}>
          Thời gian nộp:{" "}
          {moment(item.createdAtCv).format("DD-MM-YYYY HH:mm:ss")}
        </Text>
        <Text style={styles.itemText}>
        ...
        <Text style={styles.itemText}>
          Thời gian phỏng vấn:{" "}
          {item.interviewTime
            ? moment(item.interviewTime).format("DD-MM-YYYY HH:mm:ss")
            : "Chưa rõ"}
        </Text>
        <View style={styles.actionsContainer}>
          <TouchableOpacity
            onPress={() => {
              router.push(`/detailjob/${item.idPost}`);
            }}
          >
  ...
  return (
    <View style={styles.container}>
      {dataCv.length > 0 ? (
        <FlatList
          data={dataCv.sort(
            (a, b) => new Date(b.createdAtCv) - new Date(a.createdAtCv)
          )}
