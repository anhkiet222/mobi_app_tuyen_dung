...
import { getHistoryTradeCv } from "../../api/CvApi";
...
const HistoryTradeCv = () => {
  ...
  const getData = async (params) => {
    setLoading(true);
    try {
      const res = await getHistoryTradeCv(params, userToken);
      if (res && res.code === 200) {
  ...
  const renderTradeItem = ({ item, index }) => {
    return (
      <View style={styles.tradeItem}>
        <Text style={styles.tradeText}>Tên gói: {item.packageCvData.name}</Text>
        ...
        <View style={styles.cardBody}>
          ...
          >
            <TouchableOpacity
              ...
              disabled={!fromDatePost || !toDatePost}
            >
