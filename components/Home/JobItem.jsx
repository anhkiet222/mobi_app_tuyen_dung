            ...
            <TouchableOpacity onPress={() =>router.push(`/detailjob/${item.id}`)}>
                <View style={styles.itemContainer}>
                    ...
                    <View style={styles.task}>
                        <Text style={styles.textcateJob}><Foundation name="torso-business" size={13} color="white" /> {item.postDetailData.jobLevelPostData.value}</Text>
                        <Text style={styles.textExJob}><FontAwesome5 name="business-time" size={13} color="white" /> {item.postDetailData.expTypePostData.value}</Text>
                        <Text style={styles.textMoney}><FontAwesome5 name="money-bill-wave" size={13} color="white" /> {item.postDetailData.salaryTypePostData.value}</Text>
                    </View>
                    <View style={styles.task}>
                        <Text style={styles.textAdd}><FontAwesome5 name="map-marker-alt" size={13} color="white" /> {item.postDetailData.provincePostData.value}</Text>
                        <Text style={styles.textWorkJob}><FontAwesome5 name="user-clock" size={13} color="white" /> {item.postDetailData.workTypePostData.value}</Text>
                        <Text style={styles.textSex}><FontAwesome name="intersex" size={13} color="white" /> {item.postDetailData.genderPostData.value}</Text>
                    </View>
                    <View style={styles.timepost}>
                        <Text style={styles.textTime}>{handleSplitTime(item.timePost)}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        );
